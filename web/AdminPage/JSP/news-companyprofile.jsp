                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="avatar avatar-lg me-2">
                                                        <img src="<c:if test="${news.images.size() == 0}">img/General/no-image.png</c:if><c:if test="${news.images.size() > 0}">./img/UploadImgs/NewsImgs/${news.code}/${news.images.get(0).image}</c:if>" 
                                                             alt="avatar" class="img-thumbnail" style="object-fit: contain"/>
                                                        </div>
                                                    </td>
                                                        <td>${news.view}</td>
                                                <td><div><span class="badge bg-label-${news.status?"success":"danger"} me-1">${news.status?"ACTIVE":"DEACTIVE"}</span></div></td>
                                                <td>
                                                    <a title="Edit" class="btn btn-sm btn-outline-primary" href="admin-update-news?newsID=${news.newsID}">
                                                        <i class="bx bx-edit-alt me-1"></i>
                                                    </a>
                                                    <a title="${news.status?"Deactivate":"Active"}" class="btn btn-sm btn-outline-${news.status?"danger":"success"}" href="/SWP/admin-change-news-status?newsID=${news.newsID}&status=${news.status}">
                                                        <i class="bx bx-${news.status?"hide":"show"} me-1"></i>
                                                    </a>
                                    ...
                                    <ul class="pagination justify-content-end me-3">
                                        <c:if test="${size != 0}">
                                            <li class="page-item prev">
                                                <a class="page-link" href="admin-update-aboutus?page-type=${pageType}&page=${page-1==0?size:page-1}&name=${name}"><i class="tf-icon bx bx-chevrons-left"></i></a>
                                            </li>
                                        </c:if>
                                        <c:forEach begin="1" end="${size}" var="i">
                                            <li class="page-item ${i==page?"active":""}">
                                                <a class="page-link" href="admin-update-aboutus?page-type=${pageType}&page=${i}&name=${name}">${i}</a>
                                            </li>
                                        </c:forEach>
                                        <c:if test="${size != 0}">
                                            <li class="page-item next">
                                                <a class="page-link" href="admin-update-aboutus?page-type=${pageType}&page=${page+1>size?1:page+1}&name=${name}"><i class="tf-icon bx bx-chevrons-right"></i></a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </nav>

                ...
                <div class="layout-page">
                    ...
                    <div class="content-wrapper">
                        ...
                        <div class="card" style="margin: 2.5%">
                            <a href="admin-view-brand"><h5 class="card-header">TAG LIST</h5></a>
                            <div class="d-flex justify-content-between">
                                <ul class="card-body nav nav-pills">
                                    <li class="nav-item">
                                        <a href="/SWP/admin-view-tag?page-type=all"><button class="nav-link ${pageType.equals("all")?"active":""}">All</button></a>                                      
                                    </li>
                                    <li class="nav-item">
                                        <a href="/SWP/admin-search-tag?page-type=deactive"><button class="nav-link ${pageType.equals("deactive")?"active":""}">Deactive</button></a>       
                                    </li>
                                    <li class="nav-item">
                                        <a href="/SWP/admin-search-tag?page-type=active"><button class="nav-link ${pageType.equals("active")?"active":""}">Active</button></a>       
                                    </li>
                                </ul>
                                <a class="card-body d-flex justify-content-end" href="admin-create-tag"><button type="button" class="btn rounded-pill btn-danger">Add Tag</button></a>
                            </div>
                            <div class="row g-0 card-body">
                                <div class="col-6 col-md-4 d-flex"><div class="align-self-center">${TagDAO.getTagIDMax()} ${TagDAO.getTagIDMax()>1?"Tags":"Tag"}</div></div>
                                <div class="col-sm-6 col-md-8">
                                    <form action="admin-search-tag?page-type=${pageType}}" method="get">
                                        <div class="input-group input-group-merge">
                                            <span class="input-group-text" id="basic-addon-search31"><i class="bx bx-search"></i></span>
                                            <input type="text" class="form-control" name="name" placeholder="Search by tag" aria-label="Search..." aria-describedby="basic-addon-search31" />
                                        </div>
                                    </form>
                                </div>
                            </div>
                                    ...
                                    <tbody class="table-border-bottom-0">                                           
                                        <c:forEach items="${list}" var="tag">
                                            <tr>
                                                ...
                                                <td>
                                                    <a title="Edit" class="btn btn-sm btn-outline-primary" href="admin-update-tag?tagID=${tag.tag_id}">
                                                    ...
                                                    <c:if test="${tag.status}">
                                                        <a title="Deactivate" class="btn btn-sm btn-outline-danger" 
                                                           href="admin-change-tag-status?tagID=${tag.tag_id}&status=${tag.status}&page=${page}">
                                ...
                                <nav aria-label="Page navigation" class="${list.size()==0?"d-none":""}">
                                    <ul class="pagination justify-content-end me-3">
                                        <li class="page-item prev">
                                            <a class="page-link" href="/SWP/admin-search-tag?page=${page-1==0?size:page-1}"><i class="tf-icon bx bx-chevrons-left"></i></a>
                                        </li>
                                        <c:forEach begin="1" end="${size}" var="i">
                                            <li class="page-item ${i==page?"active":""}">
                                                <a class="page-link" href="/SWP/admin-search-tag?page=${i}">${i}</a>
                                            </li>
                                        </c:forEach>
                                        <li class="page-item next">
                                            <a class="page-link" href="/SWP/admin-search-tag?page=${page+1>size?1:page+1}"><i class="tf-icon bx bx-chevrons-right"></i></a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>

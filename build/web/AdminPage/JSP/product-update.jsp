                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="card mb-4">
                                        ...
                                        <!-- Product -->
                                        <form action="admin-update-product" id="formAccountSettings" method="POST" enctype="multipart/form-data">                                            
                                            <hr class="my-0" />
                                            <div class="card-body">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="mb-3 col-md-6">
                                                        ...
                                                        <input class="form-control" type="text" id="productCode"
                                                               name="productCode" value="${requestScope.product.code}" readonly/>
                                                    </div>
                                                    <div class="mb-3 col-md-6">
                                                        <label for="productName" class="form-label">Product Name</label>
                                                        <input class="form-control" type="text" name="productName"
                                                               id="productName" value="${requestScope.product.name}" autofocus/>
                                                    </div>
                                                    <div class="mb-3 col-md-6">
                                                        <label for="" class="form-label">Category</label>                                                        
                                                        <div class="dropdown">
                                                            <input id="category" type="hidden" name="cateID" value="${product.cate.cateID}" required />
                                                                ...
                                                                <li>  
                                                                    <c:if test="${category.children.size() == 0}">
                                                                        <input onclick="changeCategory(${category.cateID}, '${category.cate}')" 
                                                                           class="dropdown-item <c:if test="${product.cate.cateID eq category.cateID}">active selected</c:if>" type="button" value="${category.cate}" />
                                                                    </c:if>
                                                                    
                                                                    <c:if test="${category.children.size() != 0}">
                                                                        <input class="dropdown-item <c:forEach  items="${category.children}" var="chil"><c:if test="${product.cate.cateID eq chil.cateID}">active selected</c:if></c:forEach>" 
                                                                                   type="button" value="${category.cate} &raquo;" />
                                                                        <ul class="dropdown-menu dropdown-submenu">                                                                            
                                                                            <c:forEach  items="${category.children}" var="chil">
                                                                            <li>
                                                                                <input onclick="changeCategory(${chil.cateID}, '${chil.cate}')" 
                                                                                    class="dropdown-item <c:if test="${product.cate.cateID eq chil.cateID}">active selected</c:if>" type="button" value="${chil.cate}" />
                                                                            </li>
                                                                            </c:forEach>
                                                                        </ul>
                                                                    </c:if>
                                                    ...
                                                    <div class="mb-3 col-md-6">
                                                        ...
                                                        <select name="brandID" id="brand" class="selectpicker"
                                                            ...
                                                            <c:forEach  items="${brands}" var="brand">
                                                                <option value="${brand.brandID}" <c:if test="${brand.brandID==product.brand.brandID}">selected</c:if>>${brand.name}</option>
                                                            </c:forEach>
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="mb-3 col-md-6">
                                                        ...
                                                        <input type="text" class="form-control" id="price" name="price" 
                                                               value="${df.format(requestScope.product.price)}"/>
                                                        ...
                                                        <div class="select2-primary">                                                                                                                    
                                                            <select id="select2Multiple" name="tags" class="select2 form-select" multiple="multiple">
                                                                <c:forEach  items="${tags}" var="tag">
                                                                    <option value="tagID=${tag.tag_id}" 
                                                                            <c:forEach  items="${ptags}" var="ptag">
                                                                                <c:if test="${tag.tag==ptag.tag}">selected</c:if>
                                                                            </c:forEach>>${tag.tag}</option>
                                                                </c:forEach>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="mb-3 col-md-12">
                                                        ...
                                                        <textarea class="form-control" name="shortdescript" id="shortdescript" cols="10" rows="3">${requestScope.product.shortdescript}</textarea>
                                                    </div>
                                                    <div class="mb-3 col-md-12">
                                                        <label for="description" class="form-label">Description</label>
                                                        <textarea class="form-control" name="description" id="description" cols="10" rows="6">${requestScope.product.description}</textarea>
                                                    </div>
                                                    <input name="createdAt" type="text" style="display: none" value="${requestScope.product.createdAt}"/>
                                                    <input name="createdBy" type="text" style="display: none" value="${requestScope.product.createdBy}"/>
                                                    <input name="productID" type="text" style="display: none" value="${requestScope.product.productID}"/>
                                                </div>
                                                <div class="mt-2">
            ...
            var files = [
                <c:forEach  items="${product.images}" var="img">                           
                {
                    ...
                    options: {
                        ...
                        metadata: {
                            poster: "./img/UploadImgs/ProductImgs/${product.productID}/${img.image}"
                        }
                    }
                }, 
                </c:forEach>
